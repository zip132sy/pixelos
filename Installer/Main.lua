...
addStage(function()
	...
	-- Get original MineOS license
	local originalLicenseContent = ""
	pcall(function()
		originalLicenseContent = request("Installer/Licenses/MineOS_Original_LICENSE")
	end)
	
	if originalLicenseContent == "" then
		pcall(function()
			originalLicenseContent = request("https://raw.githubusercontent.com/IgorTimofeev/MineOS/master/LICENSE")
		end)
	end
	
	if originalLicenseContent == "" then
		originalLicenseContent = "Original MineOS License not available. Please visit https://github.com/IgorTimofeev/MineOS for more information."
	end
	
	-- Add original license toggle
	local showOriginalLicense = false
	local originalLicenseSwitch = layout:addChild(GUI.switchAndLabel(1, 1, 0x66DB80, 0xCC4940, false, localization.showOriginalLicense or "Show Original MineOS License"))
	layout.height = 3
	
	-- Add text box for license
	local licenseTextBox = layout:addChild(GUI.textBox(1, 1, layout.width, layout.height - 7, 0xF0F0F0, 0x696969, {}, 1, 1, 1))
	
	-- Update license display based on switch
	local function updateLicenseDisplay()
		if showOriginalLicense then
			local lines = text.wrap({originalLicenseContent}, layout.width - 2)
			licenseTextBox.lines = lines
		else
	...
	originalLicenseSwitch.switch.onStateChanged = function(state)
		showOriginalLicense = state
		updateLicenseDisplay()
	end
	
	-- Load default license (PixelOS)
	updateLicenseDisplay()
...
addStage(function()
	...
	-- Set filesystem proxy to selected filesystem for next boot
	filesystem.setProxy(selectedFilesystemProxy)
