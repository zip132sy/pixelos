...
local host = "https://gitee.com/zip132sy/pixelos/raw/master/AppMarket/"

-- Default app sources
local defaultSources = {
    {
        name = "MineOS Official",
        url = "https://raw.githubusercontent.com/IgorTimofeev/MineOS/master/AppMarket/",
        default = true
    },
    {
        name = "PixelOS Official",
        url = "https://gitee.com/zip132sy/pixelos/raw/master/AppMarket/",
        default = true
    },
    {
        name = "OpenPrograms",
        url = "http://openprograms.github.io/",
        default = true
    }
}
...
-- Load custom source configuration
local function loadConfig()
    if filesystem.exists(configPath) then
        local success, config = pcall(filesystem.readTable, configPath)
        if success and config then
            if config.sources then
                -- Use existing sources
                config.sources = config.sources or {}
                -- Add default sources if not present
                for _, defaultSource in ipairs(defaultSources) do
                    local exists = false
                    for _, existingSource in ipairs(config.sources) do
                        if existingSource.url == defaultSource.url then
                            exists = true
                            break
                        end
                    end
                    if not exists then
                        table.insert(config.sources, defaultSource)
                    end
                end
            else
                -- Old config format, migrate to new format
                local oldSource = config.source or host
                config.sources = {}
                for _, defaultSource in ipairs(defaultSources) do
                    table.insert(config.sources, defaultSource)
                end
                -- Add old custom source if it's not a default
                if oldSource ~= "https://gitee.com/zip132sy/pixelos/raw/master/AppMarket/" then
            ...
            if not config.currentSource then
                config.currentSource = "https://raw.githubusercontent.com/IgorTimofeev/MineOS/master/AppMarket/"
            end
            -- Update host
    ...
    for _, defaultSource in ipairs(defaultSources) do
        table.insert(defaultConfig.sources, defaultSource)
    end
    defaultConfig.currentSource = "https://raw.githubusercontent.com/IgorTimofeev/MineOS/master/AppMarket/"
    host = defaultConfig.currentSource
    return defaultConfig
end

local function saveConfig(config)
    filesystem.makeDirectory(appMarketPath)
...
local function loadConfig()
	...
	if filesystem.exists(configPath) then
		...
		-- Ensure sources are properly loaded
		if config.sources then
			-- Add default sources if not present
			for _, defaultSource in ipairs(defaultSources) do
				local exists = false
		...
		config = {
			...
			sources = {},
			currentSource = "https://gitee.com/zip132sy/pixelos/raw/master/AppMarket/"
		}
		-- Add default sources
		for _, defaultSource in ipairs(defaultSources) do
			table.insert(config.sources, defaultSource)
...
local function settings()
	...
	local function changeSource()
		...
		layout:addChild(GUI.label(1, 1, 36, 1, 0x0, localization.changeSource or "Change App Source")):setAlignment(GUI.ALIGNMENT_HORIZONTAL_CENTER, GUI.ALIGNMENT_VERTICAL_TOP)
		layout:addChild(GUI.textBox(1, 1, 36, 2, nil, 0xA5A5A5, {localization.sourceInfo or "Enter the URL of the app market source. Default is PixelOS official source."}, 1, 0, 0, true, true))
		
		-- Source list
		local sourceList = layout:addChild(GUI.list(1, 1, 36, 10, 3, 0, nil, 0x787878, nil, 0x787878, 0x2D2D2D, 0xE1E1E1, false))
		
		-- Populate source list
		for _, source in ipairs(config.sources) do
			local item = sourceList:addItem(source.name)
			item.onTouch = function()
				config.currentSource = source.url
				host = source.url
				saveConfig()
				-- Update list selection
				for i, listItem in ipairs(sourceList.children) do
					listItem.selected = listItem.text == source.name
				end
				workspace:draw()
			end
			-- Mark current source as selected
			if source.url == config.currentSource then
				sourceList.selectedItem = #sourceList.children
			end
		end
		
		-- Add source controls
		local nameInput = layout:addChild(GUI.input(1, 1, 17, 3, 0xFFFFFF, 0x696969, 0xB4B4B4, 0xFFFFFF, 0x2D2D2D, "", "Source Name"))
		local urlInput = layout:addChild(GUI.input(19, 1, 18, 3, 0xFFFFFF, 0x696969, 0xB4B4B4, 0xFFFFFF, 0x2D2D2D, "", "Source URL"))
		
		local buttonsLayout = layout:addChild(newButtonsLayout(1, 1, 36, 2))
		
		buttonsLayout:addChild(GUI.adaptiveRoundedButton(1, 1, 2, 0, 0x66DB80, 0xFFFFFF, 0x2D2D2D, 0xFFFFFF, "Add")).onTouch = function()
			if nameInput.text and urlInput.text and #nameInput.text > 0 and #urlInput.text > 0 then
				-- Check if source already exists
				local exists = false
				for _, source in ipairs(config.sources) do
					if source.url == urlInput.text then
						exists = true
						break
					end
				end
				...
				if not exists then
					...
					GUI.alert("Source already exists")
				end
			else
				GUI.alert("Please enter source name and URL")
			end
		end
		
		buttonsLayout:addChild(GUI.adaptiveRoundedButton(1, 1, 2, 0, 0xFF6B6B, 0xFFFFFF, 0x2D2D2D, 0xFFFFFF, "Remove")).onTouch = function()
			if sourceList.selectedItem then
				local selectedSource = config.sources[sourceList.selectedItem]
				if selectedSource and not selectedSource.default then
					-- Remove source
					table.remove(config.sources, sourceList.selectedItem)
					-- If we removed the current source, switch to the first one
					if config.currentSource == selectedSource.url then
						config.currentSource = config.sources[1].url
						host = config.currentSource
					end
					saveConfig()
					-- Refresh source list
					...
					GUI.alert("Cannot remove default sources")
				end
			else
				GUI.alert("Please select a source to remove")
			end
		end
	...
	-- Common settings (available without login)
	contentContainer:removeChildren()
	local layout = contentContainer:addChild(GUI.layout(1, 1, contentContainer.width, contentContainer.height, 1, 1))
	...
	for key, value in pairs(languages) do
		languageComboBox:addItem(value).onTouch = function()
			...
			saveConfig()
			-- Reload localization after language change
			localization = system.getLocalization()
	...
	buttonsLayout:addChild(GUI.adaptiveRoundedButton(1, 1, 2, 0, 0xC3C3C3, 0xFFFFFF, 0x2D2D2D, 0xFFFFFF, localization.changeSource or "Change Source")).onTouch = changeSource

	-- User-specific settings (only when logged in)
	if user.token then
...
local function dialogGUI(to_user_name)
	...
	local function check()
		button.disabled = not (to_user_name and input.text and #input.text > 0)
	end
...
editPublication = function(initialPublication, initialCategoryID)
	...
	local function addInput(text, placeholder, multiline)
		return layout:addChild(GUI.input(1, 1, 36, multiline and 3 or 1, 0xFFFFFF, 0x878787, 0xC3C3C3, 0xFFFFFF, 0x2D2D2D, text, placeholder, false, nil))
	end
