...
module.onTouch = function()
	...
	local readonlySwitch = window.contentLayout:addChild(GUI.switchAndLabel(1, 1, 36, 8, 0x66DB80, 0x2D2D2D, 0xE1E1E1, 0xA5A5A5, localization.disksReadOnly .. ":", false))
	...
	local function update()
		...
		readonlySwitch.switch.state = proxy.isReadOnly()
	end
	
	readonlySwitch.switch.onStateChanged = function()
		local proxy = getProxy()
		local success, reason
		if proxy.setReadOnly then
			success, reason = pcall(proxy.setReadOnly, readonlySwitch.switch.state)
		else
			success, reason = false, localization.disksReadOnlyNotSupported or "This filesystem does not support changing read-only mode"
		end
		if not success then
			GUI.alert(reason or localization.operationFailed or "Operation failed")
			readonlySwitch.switch.state = not readonlySwitch.switch.state
		end
		update()
		workspace:draw()
	end
