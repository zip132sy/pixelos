...
function system.getDefaultUserSettings()
	return {
		localizationLanguage = "ChineseSimplified",
		...
		interfaceStatusBarShowCPU = true,
...
function system.getLocalization(pathToLocalizationFolder)
	-- 首先尝试从系统语言文件加载
	local localizationLanguage = userSettings.localizationLanguage
	local systemLocalization = filesystem.readTable(paths.system.localizations .. localizationLanguage .. ".lang") or {}
	
	-- 然后尝试从本地语言文件加载，覆盖系统语言文件中的值
	if filesystem.exists(pathToLocalizationFolder) then
		if not filesystem.exists(pathToLocalizationFolder .. localizationLanguage .. ".lang") then
			if filesystem.exists(pathToLocalizationFolder .. "Chinese.lang") then
				localizationLanguage = "Chinese"
			end
		end
		
		if localizationLanguage == "Chinese" then
			if userSettings.localizationLanguage == "ChineseSimplified" and filesystem.exists(pathToLocalizationFolder .. "ChineseSimplified.lang") then
				localizationLanguage = "ChineseSimplified"
			elseif userSettings.localizationLanguage == "ChineseTraditional" and filesystem.exists(pathToLocalizationFolder .. "ChineseTraditional.lang") then
				localizationLanguage = "ChineseTraditional"
			elseif filesystem.exists(pathToLocalizationFolder .. "ChineseSimplified.lang") then
				localizationLanguage = "ChineseSimplified"
			end
		end
		
		local required, english = pathToLocalizationFolder .. localizationLanguage .. ".lang", pathToLocalizationFolder .. "English.lang"
		local localLocalization
		...
		if filesystem.exists(required) then
			...
			localLocalization = requestedLocalization
		else
			localLocalization = firstAvailable
		end
		
		-- 合并本地语言文件到系统语言文件
		for k, v in pairs(localLocalization) do
			systemLocalization[k] = v
		end
	end

	-- For unknown localization name cases
	setmetatable(systemLocalization, {__index=(function(t,k) return "$"..k end)})
...
-- 合并所有应用程序的语言文件到系统语言文件
function system.mergeLocalizations()
	print("合并应用程序语言文件到系统语言文件...")
	
	-- 获取所有系统语言文件
	local systemLocalesPath = paths.system.localizations
	local systemLocales = {}
	
	for file in filesystem.list(systemLocalesPath) do
		local localeName = filesystem.hideExtension(file)
		local filePath = systemLocalesPath .. file
		local content = filesystem.read(filePath)
		if content then
			local success, locale = pcall(load("return " .. content))
			if success and locale then
				systemLocales[localeName] = locale
			end
		end
	end
	
	-- 扫描所有应用程序的语言文件
	local appsPath = paths.system.applications
	
	for appDir in filesystem.list(appsPath) do
		local appPath = appsPath .. appDir .. "/"
		local localesPath = appPath .. "Localizations/"
		
		if filesystem.exists(localesPath) then
			print("处理应用程序: " .. appDir)
			
			-- 获取应用程序名称（去除.app后缀）
			local appName = filesystem.hideExtension(appDir)
			-- 创建应用程序的键前缀
			local keyPrefix = appName:lower():gsub("%s+", "_") .. "_"
			
			-- 处理每个语言文件
			for localeFile in filesystem.list(localesPath) do
				local localeName = filesystem.hideExtension(localeFile)
				local filePath = localesPath .. localeFile
				local content = filesystem.read(filePath)
				
				if content then
					local success, appLocale = pcall(load("return " .. content))
					if success and appLocale then
						-- 确保系统语言文件存在
						if not systemLocales[localeName] then
							systemLocales[localeName] = {}
						end
						
						-- 合并应用程序的翻译到系统语言文件
						for key, value in pairs(appLocale) do
							local newKey = keyPrefix .. key
							systemLocales[localeName][newKey] = value
						end
					end
				end
			end
	...
	-- 保存合并后的系统语言文件
	for localeName, locale in pairs(systemLocales) do
		local filePath = systemLocalesPath .. localeName .. ".lang"
		local content = "{\n"
		...
		for i, key in ipairs(keys) do
			...
			content = content .. "\t" .. key .. " = \"" .. value:gsub("\"", "\\\"") .. "\""
			if i < #keys then
				content = content .. ","
			end
			content = content .. "\n"
...
local function addMainDesktopMenuItem(menu)
	...
	item:addItem("🛈", localization.aboutSystem).onTouch = function()
		...
		local cpuInfo = "Unknown"
		for address, ctype in component.list("computer") do
			local proxy = component.proxy(address)
			if proxy then
				cpuInfo = proxy.address:sub(1, 8) .. "..."
			end
		end
		
		local ramTotal = computer.totalMemory() / 1024
		local ramUsed = (computer.totalMemory() - computer.freeMemory()) / 1024
		
		local diskInfo = {}
		for address, ctype in component.list("filesystem") do
			local proxy = component.proxy(address)
			local label = proxy.getLabel() or "Unlabeled"
			local total = proxy.spaceTotal() or 0
			local used = proxy.spaceUsed() or 0
			table.insert(diskInfo, {label = label, total = total, used = used})
		end
		
		local lines = {
			"PixelOS System Information",
			"========================",
			" ",
			"CPU Architecture: " .. cpuInfo,
			"RAM Total: " .. string.format("%.1f KB", ramTotal),
			"RAM Used: " .. string.format("%.1f KB", ramUsed),
			"RAM Free: " .. string.format("%.1f KB", ramTotal - ramUsed)
		}
		
		if #diskInfo > 0 then
			table.insert(lines, " ")
			table.insert(lines, "Disks:")
			for _, disk in ipairs(diskInfo) do
				table.insert(lines, "  " .. disk.label .. ": " .. disk.used .. "/" .. disk.total .. " bytes")
			end
		end
		...
		table.insert(lines, "========================")
		table.insert(lines, " ")
		table.insert(lines, "PixelOS is based on MineOS")
		table.insert(lines, " ")
		table.insert(lines, "Original MineOS Authors:")
		table.insert(lines, "  Igor Timofeev")
		table.insert(lines, "  Gleb Trifonov")
		table.insert(lines, "  Fedor Cheremisenov")
		table.insert(lines, "  Yakov Verevkin")
		table.insert(lines, "  Alexey Smirnov")
		table.insert(lines, "  Timofey Shestakov")
		table.insert(lines, "  Alexander Fursenko")
		table.insert(lines, " ")
		table.insert(lines, "Original MineOS:")
		table.insert(lines, "  https://github.com/MightyPirates/OpenComputers")
		table.insert(lines, "PixelOS:")
		table.insert(lines, "  https://gitee.com/zip132sy/pixelos")
	...
	item:addItem("🛒", localization.updates).onTouch = function()
		system.execute(paths.system.applicationSystemUpdate, "")
	end
...
local function addBackgroundContainerInput(parent, text, placeholderText, eraseTextOnFocus, textMask)
	return parent:addChild(GUI.input(1, 1, 36, 3, 0xE1E1E1, 0x696969, 0x969696, 0xE1E1E1, 0x2D2D2D, text, placeholderText, eraseTextOnFocus, textMask))
end
...
local function addBackgroundContainerWithInput(inputText, title, placeholder)
	...
	container.input = addBackgroundContainerInput(container.layout, inputText, placeholder, false, nil)
...
function system.launchWithArguments(path)
	local container = addBackgroundContainerWithInput("", localization.launchWithArguments, localization.arguments)
		...
		if icon.isDirectory then
			...
			if icon.extension ~= ".app" then
				contextMenu:addItem("⭐", localization.addToFavourites).onTouch = function()
					...
					local input = addBackgroundContainerInput(container.layout, icon.name, localization.name, false, nil)
					container.panel.eventHandler = function(workspace, object, e1)
						if e1 == "touch" then
							container:remove()
...
local function iconFieldBackgroundClick(iconField, e1, e2, e3, e4, e5, ...)
	...
	subMenu:addItem("📷", localization.newImage).onTouch = function()
		...
		local widthInput = addBackgroundContainerInput(layout, "", "width", false, nil)
		layout:addChild(GUI.text(1, 1, 0x696969, " x "))
		local heightInput = addBackgroundContainerInput(layout, "", "height", false, nil)
	...
	subMenu:addItem("🌍", localization.newFileFromURL, not component.isAvailable("internet")).onTouch = function()
		...
		local inputURL = addBackgroundContainerInput(container.layout, "", "URL", false, nil)
...
function system.updateDesktop()
	...
	dockContainer.addIcon = function(path)
		-- Check new attachment to dock, if it's a shortcut, then look for original file and attach it
	...
	dateWidget.draw = function(dateWidget)
		if userSettings.interfaceStatusBarEnabled == false then
			return
		end
	...
	batteryWidget.draw = function(batteryWidget)
		if userSettings.interfaceStatusBarEnabled == false or userSettings.interfaceStatusBarShowBattery == false then
			return
		end
	...
	RAMWidget.draw = function(RAMWidget)
		if userSettings.interfaceStatusBarEnabled == false or userSettings.interfaceStatusBarShowRAM == false then
			return
		end
		local text = (localization.RAM or "RAM") .. ": " .. math.ceil(RAMPercent * 100) .. "% "
	...
	local function updateDiskWidgets()
		-- Clear old disk widgets
		for i = #diskWidgets, 1, -1 do
			diskWidgets[i].widget:remove()
			diskWidgets[i] = nil
		end
		...
		for address, ctype in component.list("filesystem") do
			...
			table.insert(filesystems, {
				...
				total = total,
				used = used,
				percent = total > 0 and used / total or 0
			})
		end
		
		-- Create disk widgets for each filesystem
		for i, fs in ipairs(filesystems) do
			...
			diskWidget.draw = function(diskWidget)
				if userSettings.interfaceStatusBarEnabled == false or userSettings.interfaceStatusBarShowDisk == false then
					return
				end
				
				-- Check if this disk should be shown (based on user settings)
				local diskKey = "disk_" .. i
				if userSettings.interfaceStatusBarShowDisks and userSettings.interfaceStatusBarShowDisks[diskKey] == false then
					return
				end
				
				local text = (fs.label or (localization.disk or "Disk") .. " " .. i) .. ": " .. math.ceil(fs.percent * 100) .. "% "
				local barWidth = diskWidget.width - #text
				local activeWidth = math.ceil(fs.percent * barWidth)

				screen.drawText(diskWidget.x, 1, 0x787878, text)
				
				local index = screen.getIndex(diskWidget.x + #text, 1)
				
				for j = 1, barWidth do
					screen.rawSet(index, screen.rawGet(index), j <= activeWidth and getPercentageColor(1 - fs.percent) or 0x3C3C3C, "━")
					index = index + 1
				end
			end
			table.insert(diskWidgets, {widget = diskWidget, fs = fs})
		end
	end
	
	-- Initialize disk widgets
	updateDiskWidgets()

	local cpuWidget = system.addMenuWidget(system.menuWidget(12))
	local cpuArch = "OC"
	cpuWidget.draw = function(cpuWidget)
		if userSettings.interfaceStatusBarEnabled == false or userSettings.interfaceStatusBarShowCPU == false then
			return
		end
		local text = (localization.cpu or "CPU") .. ": " .. cpuArch .. " "
